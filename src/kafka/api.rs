...
pub fn handle_api_key_18(connection: &mut Connection) -> Result<([u8; 1016], i32), io::Error> {
    ...
    for api in supported_apis {
        ...
        start_index += 2;
        response_body[start_index..start_index + 2].copy_from_slice(&api.min_version.to_be_bytes());
        start_index += 2;
        response_body[start_index..start_index + 2].copy_from_slice(&api.max_version.to_be_bytes());
        start_index += 2;
...
fn parse_body(connection: &mut Connection) -> Result<(), io::Error> {
    ...
    for _ in 0..topic_array_length {
        ...
        let topic_name = match String::from_utf8(
            connection.body[current_index..current_index + topic_name_length as usize].to_vec(),
        ) {
    ...
    connection.partition_limit = i32::from_be_bytes(
        connection.body[current_index..current_index + 4]
            .try_into()
            .unwrap(),
    );
    current_index += 4;
...
pub fn handle_api_key_75(connection: &mut Connection) -> Result<([u8; 1016], i32), io::Error> {
    ...
    Ok((response_body, start_index as i32))
}
